...
contract NexternalBribe is IBribe {
    ...
    // NexternalBribe public underlying_bribe; // removed this as we just add back in the block it needs to work.
    ...
    uint256 public totalSupply; // adding this back also
    mapping(uint256 => uint256) public balanceOf; // adding this back even though it seems we dont need it
    ...
    // adding this back in from external also
    ...
    mapping(uint256 => mapping(uint256 => RewardCheckpoint)) public checkpoints;
    ...
    // this part is getting added back in so we dont have to call the other contract with underlying_bribe
    ...
    function getPriorBalanceIndex(uint256 tokenId, uint256 timestamp) public view returns (uint256) {
        ...
        while (upper > lower) {
            ...
            RewardCheckpoint memory cp = checkpoints[tokenId][center];
    ...
    // added from xbribe will replace underlying_bribe so it does not need to reference the other contract at all?
