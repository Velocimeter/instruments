...
async function getClosestBlock(timestamp) {
  ...
  if (foundExactBlock) {
    console.log(
      `found a block that exactly matches the timestamp=${timestamp}, closestBlockNumber=${closestBlockNumber}, blockTimestamp=${closestBlock.timestamp}`
    );
    console.log(
      `adjacent block timestamps: leftBlockTimestamp=${previousBlock.timestamp}, rightBlockTimestamp=${nextBlock.timestamp}`
    );
  } else {
    console.log(
      `did not find a block that exactly matches the timestamp=${timestamp}, potential closestBlockNumber=${closestBlockNumber}, blockTimestamp=${closestBlock.timestamp}`
    );
    console.log(
      `adjacent block timestamps: leftBlockTimestamp=${previousBlock.timestamp}, rightBlockTimestamp=${nextBlock.timestamp}`
    );
