...
task('batch-create-lock-for', 'Creates lock for a recipient address')
  ...
  .setAction(async (taskArguments, { ethers }) => {
    ...
    const approveTx = await (
      await flow.approve(veContract, ethers.constants.MaxUint256)
    ).wait()
    ...
    for (let d of createLockDetailsList) {
      ...
      console.log(
        `Creating Lock of ${amountInMillion} million for ${recipient} for ${durationInYear} years`
      )

      try {
        const createLockForTx = await (
          await escrow.create_lock_for(amount, duration, recipient)
        ).wait()
